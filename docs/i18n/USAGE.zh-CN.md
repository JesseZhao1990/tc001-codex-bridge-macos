...
- 打开“显示桌面卡片”后，会出现额度、模型状态、模型来源、TC001 连接、电量和最近同步状态的汇总卡片。
- 按住卡片背景即可拖动，应用会记住上次停放位置。
- 使用卡片右上角的图钉，或设置中的“桌面卡片始终置顶”，可在置顶卡片和普通窗口之间切换。
- 使用卡片关闭按钮或顶部菜单弹窗可隐藏卡片；之后可以从设置或顶部菜单弹窗重新显示。
- 5 小时与 7 天开关会同时控制 TC001、顶部菜单弹窗、菜单栏文字和桌面卡片。
