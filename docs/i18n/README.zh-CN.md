...
应用会显示左右额度条、5H/7D 数字和四色状态灯。5 小时与 7 天额度可分别开关，支持只显示其中一种或同时显示两种。macOS 顶部菜单可直接显示启用的额度；原生 WidgetKit 桌面小组件提供小号和中号两种尺寸，集中呈现额度、模型工作状态、模型来源、TC001 连接、电量和最近同步状态，位置和尺寸由 macOS 管理。还可配置时间、日期、温度、湿度、电量页面。所有 Codex 状态分析都在 Mac 本地完成。

## 环境要求

应用需要 macOS 13 或更高版本，桌面小组件需要 macOS 14 或更高版本。还需要已登录的 Codex 桌面应用或 CLI，以及运行 AWTRIX 3 的 TC001。使用蓝牙时必须刷入配套的 awtrix3-ble 固件。
...
4. 在桌面空白处点按右键，选择“编辑小组件”，搜索“TC001 Bridge”并添加小号或中号组件。
