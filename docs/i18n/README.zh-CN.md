...
应用会显示左右额度条、5H/7D 数字和四色状态灯。5 小时与 7 天额度可分别开关，支持只显示其中一种或同时显示两种。macOS 顶部菜单可直接显示启用的额度；精美的可拖动桌面卡片会集中呈现额度、模型工作状态、模型来源、TC001 连接、电量和最近同步状态，并支持隐藏与始终置顶。还可配置时间、日期、温度、湿度、电量页面。所有 Codex 状态分析都在 Mac 本地完成。
